...
public class AsyncDataScheduler {
	...
	public AsyncDataScheduler(AdapterView<?> adapterView,int maxThreadCount,AsyncDataExecutor executor){
		...
		if(maxThreadCount <= 0) throw new IllegalArgumentException("maxThreadCount should be great than 0.");
	...
	public void setExtraCountForExecutingData(int extraCount){
		if(extraCount < 0) throw new IllegalArgumentException("extraCount should be great than 0 or equals 0.");
		mExtraCountForExecutingData = extraCount;
	}
	
	/**
	 * <p>设置额外的(可见范围之外的)保持异步数据不被回收的个数
	 * <p>该个数的设置有限制，既是出于内存不足的考虑，也是因为置软引用需要遍历的列表不能太大
	 * @param extraCount
	 */
	public void setExtraCountForKeepingData(int extraCount){
		if(extraCount < 0 || extraCount > 50) throw new IllegalArgumentException("extraCount should be between 0 and 50.");
